...
from cryo_particle_pretrain import MRC2DDataset  # 新しいデータセットクラスをインポート
...
def get_args_parser():
    ...
    parser.add_argument('--input_file', default='grandmodel.mrc', type=str,
...
def main(args):
    ...
    if misc.is_main_process():
        wandb.init(project="2dmae_cryo-et-particle-masking", config=vars(args))
    ...
    transform_train = transforms.Compose([
        transforms.Resize((224, 224), transforms.InterpolationMode.BICUBIC, antialias=True),  # 224x224にリサイズ
        # transforms.RandomResizedCrop(224, scale=(0.2, 1.0), interpolation=3, antialias=True),  # 0.2-1.0の範囲をランダムに切り抜いてリサイズ
    ...
    if args.use_all_tomograms:
        ...
        for i in range(10):  # model_0_particles から model_9_particles まで
            model_path = os.path.join(base_data_path, f'model_{i}_particles', '1BXN')
            input_files = [f for f in os.listdir(model_path) if f.endswith(args.input_file)]
            for input_file in input_files:
                input_path = os.path.join(model_path, input_file)
                target_path = os.path.join(model_path, input_file.replace(args.input_file, args.target_file))
        ...
        base_data_path = os.path.dirname(args.data_path)
        model_path = os.path.join(base_data_path, f'model_0_particles', '1BXN')
        input_files = [f for f in os.listdir(model_path) if f.endswith(args.input_file)]
        datasets_list = []
        for input_file in input_files:
            input_path = os.path.join(model_path, input_file)
